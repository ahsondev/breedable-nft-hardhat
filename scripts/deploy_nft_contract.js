...
const func = async function (hre) {
  ...
  switch (metadata_location) {
    case GITHUB:
      ...
      if (PREDEFINED_GITHUB_GIST_ID) {
        console.log(
          'using predefined github gist: ' + PREDEFINED_GITHUB_GIST_ID
        )
      ...
      } // load it from the temp metadata files
      else {
    ...
    case PINATA:
      {
        ...
        baseMetadataUri =
          ERC721Config.gatewayUrl + '/' + ERC721Config.metadataHash + '/'
        contractUri =
          ERC721Config.gatewayUrl + '/' + ERC721Config.contractUriHash
  ...
  await deploy('Iceberg', {
    from: deployer,
    args: [baseMetadataUri, tokenAmount, VRFCoordinator, LinkToken, keyhash],
    log: true,
  })
}
